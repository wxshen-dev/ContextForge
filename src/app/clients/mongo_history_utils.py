...
class HistoryMongoTool:
    ...
    def __init__(self):
        ...
        """
        try:
            # Read the MongoDB connection URL from environment variables.
            ...
            # Get the conversation-history collection, similar to a table in relational databases.
            self.chat_message = self.db["chat_message"]

            # Create a compound index to improve session history queries.
            # Index rule: session_id ascending + ts descending for "latest records by session".
            # create_index is idempotent and does not duplicate existing indexes.
...
# Global singleton for HistoryMongoTool.
_history_mongo_tool = None

# Warm up the singleton during module import; lazy loading can retry if this fails.
try:
...
def save_chat_message(
    ...
    :param image_urls: Optional related image URL list.
